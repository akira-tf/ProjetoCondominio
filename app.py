...
# Cache para os dados da planilha
@st.cache_data(ttl=600)  # ttl=600 significa que os dados ficam em cache por 10 minutos
def load_data():
    """Carrega os dados da planilha do Google Sheets e os processa."""
    try:
        # Autorização para aceder à planilha
        ...
        if 'EXPEDIDA' in df.columns:
            ...
            st.error("Coluna 'EXPEDIDA' não encontrada na planilha.")
            return pd.DataFrame()
        ...
        st.error("Ficheiro 'credentials.json' não encontrado. Faça o upload deste ficheiro para o seu repositório do GitHub junto com o app.py.")
        return None
...
if df_original is not None and not df_original.empty:
    ...
    with col_graf1:
        ...
        fig_bar = px.bar(
            top_5_motivos,
            x=top_5_motivos.index,
            y=top_5_motivos.values,
            title="Top 5 Motivos de Notificação",
    ...
    with col_graf2:
        ...
        fig_line = px.line(
            notificacoes_por_mes,
            x='MES_ANO',
            y='Contagem',
            title="Tendência de Notificações por Mês",
            labels={'MES_ANO': 'Mês/Ano', 'Contagem': 'Quantidade de Notificações'},
            markers=True
        )
    ...
    user_input = st.text_area(
        ...
        height=150,
        placeholder="Ex: Morador reclama de vazamento no teto da casa de banho da unidade..."
    )
